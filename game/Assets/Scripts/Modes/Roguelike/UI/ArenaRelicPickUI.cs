...
{
  /// <summary>A14: Boss kill 3-pick Relic UI (in-run rule changes).</summary>
  [DisallowMultipleComponent]
  public sealed class ArenaRelicPickUI : MonoBehaviour
  {
    const int SortOrder = 880;

    static ArenaRelicPickUI s_instance;

    CanvasGroup _group;
    RectTransform _panel;
    readonly Button[] _buttons = new Button[3];
    readonly Text[] _labels = new Text[3];
    ArenaRelicRuntime.RelicDef[] _offer;
    Text _titleLabel;
    string _timelineTag = "Boss Relic";
    bool _showing;
    ...
    public static void EnsureExists()
    {
      if (s_instance != null)
        return;
      var go = new GameObject("_ArenaRelicPickUI");
      DontDestroyOnLoad(go);
      s_instance = go.AddComponent<ArenaRelicPickUI>();
    }

    public static bool IsShowing => s_instance != null && s_instance._showing;

    public static void ShowOffer(string title = null, string timelineTag = "Boss Relic")
    {
      EnsureExists();
    ...
    void BuildUI()
    {
      ...
      var overlay = CreateImage(transform, "Overlay", new Color(0.02f, 0.04f, 0.06f, 0.78f),
        Vector2.zero, Vector2.one, Vector2.zero, Vector2.zero);

      _panel = CreateImage(overlay.transform, "Panel", new Color(0.06f, 0.1f, 0.14f, 0.96f),
        new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), Vector2.zero, new Vector2(720f, 420f)).rectTransform;

      _titleLabel = CreateLabel(_panel, "Title", "Boss 遗物 — 选择一项规则强化", 28, FontStyle.Bold,
        new Vector2(0.5f, 1f), new Vector2(0.5f, 1f), new Vector2(0f, -36f), new Vector2(640f, 40f));

      for (var i = 0; i < 3; i++)
      {
        var x = -220f + i * 220f;
        var btnRt = CreateImage(_panel, $"RelicBtn_{i}", new Color(0.1f, 0.18f, 0.22f, 0.95f),
    ...
    void ShowInternal(string title, string timelineTag)
    {
      ...
      _timelineTag = string.IsNullOrEmpty(timelineTag) ? "Boss Relic" : timelineTag;
      if (_titleLabel != null)
        _titleLabel.text = string.IsNullOrEmpty(title) ? "Boss 遗物 — 选择一项规则强化" : title;

      _offer = ArenaRelicRuntime.RollOffer(3);
      for (var i = 0; i < 3; i++)
      {
        var relic = _offer != null && i < _offer.Length ? _offer[i] : null;
        var visible = relic != null;
        _buttons[i].gameObject.SetActive(visible);
        if (!visible)
          continue;
        _labels[i].text = $"<b>{relic.display_name}</b>\n\n{relic.description}";
      }
    ...
    void Pick(int index)
    {
      ...
      ArenaRelicRuntime.ApplyRelic(_offer[index]);
